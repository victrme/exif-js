...
export class Exifjs {
	...
	async getData(img) {
		...
		if (isFile) {
			...
			const data = await new Promise((resolve) => {
				fileReader.addEventListener("load", (event) => {
					...
					const result = Exifjs.handleBinaryFile(file)
					resolve(result)
				})
		...
		if (img.src.startsWith("data:")) {
			const arrayBuffer = Exifjs.base64ToArrayBuffer(img.src)
			const data = Exifjs.handleBinaryFile(arrayBuffer)
		...
		if (img.src.startsWith("blob:")) {
			...
			const data = await new Promise((resolve) => {
				fileReader.addEventListener("load", (event) => {
					const file = event.target.result
					const response = Exifjs.handleBinaryFile(file)
					resolve(response)
				})
			})
		...
		const data = Exifjs.handleBinaryFile(buffer)
	...
	readFromBinaryFile(file) {
		return Exifjs.findEXIFinJPEG(file)
	}
	...
	static readTagValue(file, entryOffset, tiffStart, _dirStart, bigEnd) {
		...
		switch (type) {
			...
			case 2: // ascii, 8-bit byte
				offset = numValues > 4 ? valueOffset : entryOffset + 8
				return Exifjs.getStringFromDB(file, offset, numValues - 1)
	 ...
	 * @param {number} bigEnd
	 */
	static getNextIFDOffset(dataView, dirStart, bigEnd) {
	 ...
	 * @returns {object}
	 */
	static readEXIFData(dataview, start) {
		if (Exifjs.getStringFromDB(dataview, start, 4) !== "Exif") {
			return {}
		}
		...
		if (tags.ExifIFDPointer) {
			...
			for (tag in exifData) {
				switch (tag) {
					...
					case "FileSource":
						exifData[tag] = Exifjs.StringValues[tag][exifData[tag]]
						break
					...
					case "ComponentsConfiguration":
						exifData[tag] = Exifjs.StringValues.Components[exifData[tag][0]] +
							Exifjs.StringValues.Components[exifData[tag][1]] +
							Exifjs.StringValues.Components[exifData[tag][2]] +
							Exifjs.StringValues.Components[exifData[tag][3]]
						break
				}
	...
	static findXMPinJPEG(file) {
		...
		while (offset < length - 4) {
			if (Exifjs.getStringFromDB(dataView, offset, 4) === "http") {
				...
				return Exifjs.xml2Object(domDocument)
			} else {
				offset++
			}
		}
		...
		if (xml.hasChildNodes()) {
			for (let i = 0; i < xml.childNodes.length; i++) {
				...
				if (json[nodeName] === null) {
					json[nodeName] = Exifjs.xml2json(child)
				} else {
					...
					json[nodeName].push(Exifjs.xml2json(child))
				}
			}
		}
		...
		if (xml.children.length > 0) {
			for (let i = 0; i < xml.children.length; i++) {
				...
				if (typeof obj[nodeName] === "undefined") {
					obj[nodeName] = Exifjs.xml2json(item)
				} else {
					...
					obj[nodeName].push(Exifjs.xml2json(item))
				}
			}
		} else {
			obj = xml.textContent
